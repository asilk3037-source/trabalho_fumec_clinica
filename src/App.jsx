...
/* NAV */
.nav { background:var(--ink); position:sticky; top:0; z-index:300; height:64px; padding:0 2.5rem; display:flex; align-items:center; justify-content:space-between; border-bottom:none; }
...
.btn { display:inline-flex; align-items:center; gap:7px; padding:12px 26px; border-radius:var(--r-sm); font-size:14.5px; font-weight:600; cursor:pointer; border:none; transition:all .18s; font-family:var(--font-body); text-decoration:none; line-height:1; }
...
/* CRISIS */
.crisis-box { background:linear-gradient(135deg,#7F1D1D,#991B1B); border-radius:var(--r); padding:1.75rem 2rem; margin-top:2rem; display:flex; gap:1.25rem; align-items:flex-start; width:100%; max-width:100%; }
...
.contact-info h3 { font-family:var(--font-head); font-size:18px; font-weight:700; margin-bottom:1.5rem; color:var(--ink); }
...
.fg input,.fg select,.fg textarea { width:100%; padding:10px 13px; border:1.5px solid var(--border); border-radius:var(--r-sm); font-size:14px; color:var(--ink); background:var(--bg); outline:none; transition:border-color .18s,box-shadow .18s; font-family:var(--font-body); }
...
.msg-ok { background:var(--green-bg); border:1px solid #A7D7D2; border-radius:var(--r-sm); padding:.9rem 1.1rem; font-size:13.5px; color:var(--green); margin-top:1rem; }
.msg-err { background:var(--red-bg); border:1px solid #F5C6C2; border-radius:var(--r-sm); padding:.9rem 1.1rem; font-size:13.5px; color:var(--red); margin-top:1rem; }

/* FOOTER */
footer { background:#07203A; color:rgba(255,255,255,.65); padding:3.5rem 2.5rem 1.75rem; border-top:1px solid rgba(255,255,255,.06); }
...
.modal-logo { display:flex; align-items:center; gap:10px; margin-bottom:1.5rem; }
...
.toast-item { padding:12px 18px; border-radius:var(--r-sm); font-size:13.5px; font-weight:600; box-shadow:var(--shadow-lg); animation:slideIn .25s ease; pointer-events:auto; display:flex; align-items:center; gap:10px; min-width:260px; }
...
.dash-main { flex:1; background:var(--bg-warm); padding:2rem 2.5rem; overflow-y:auto; }
...
.kpi-row { display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:14px; margin-bottom:2rem; }
...
/* STATUS */
.st { display:inline-block; padding:3px 10px; border-radius:100px; font-size:11.5px; font-weight:700; letter-spacing:.3px; }
...
/* PROFILE */
.prof-card { display:flex; align-items:center; gap:1.5rem; background:var(--bg); border:1px solid var(--border); border-radius:var(--r); padding:1.5rem; margin-bottom:1.5rem; }
.prof-av-wrap { position:relative; flex-shrink:0; }
.prof-av { width:80px; height:80px; border-radius:16px; background:var(--teal); color:#fff; display:flex; align-items:center; justify-content:center; font-size:34px; font-weight:800; font-family:var(--font-head); overflow:hidden; }
.prof-av img { width:100%; height:100%; object-fit:cover; }
.prof-av-edit { position:absolute; bottom:-4px; right:-4px; width:26px; height:26px; border-radius:50%; background:var(--teal); color:#fff; border:2px solid var(--bg); display:flex; align-items:center; justify-content:center; font-size:13px; cursor:pointer; transition:background .15s; }
.prof-av-edit:hover { background:var(--teal-mid); }
...
.fb-hdr { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:7px; }
...
/* NOTIF BADGE */
.notif-dot { display:inline-block; width:8px; height:8px; border-radius:50%; background:#EF4444; margin-left:4px; }
...
.user-info-sub { font-size:12.5px; color:var(--ink-3); }
...
.msg-card-hdr { display:flex; justify-content:space-between; align-items:center; margin-bottom:6px; }
.msg-card-from { font-family:var(--font-head); font-size:14px; font-weight:700; color:var(--ink); }
.msg-card-time { font-size:12px; color:var(--ink-3); }
...
@media(max-width:768px){ .sidebar{display:none;} .hide-m{display:none;} }
`;
...
export default function App() {
  ...
  async function doRegister() {
    ...
    const statusInicial = role === "supervisor" ? "pendente_aprovacao" : "ativo";
    ...
    if (role === "supervisor") {
      toast("⏳ Conta criada! Aguardando aprovação de um supervisor.", "info");
    } else {
      setPage("dashboard");
        ...
        <>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"calc(100vh - 64px)",padding:"2rem"}}>
            <div style={{textAlign:"center",maxWidth:480}}>
              ...
              <h2 style={{fontFamily:"var(--font-head)",fontSize:24,marginBottom:"1rem",color:"var(--ink)"}}>Aguardando aprovação</h2>
              <p style={{color:"var(--ink-2)",marginBottom:"2rem",lineHeight:1.7}}>Sua conta de supervisor foi criada e está aguardando aprovação de outro supervisor já cadastrado. Você receberá acesso assim que for aprovado.</p>
    ...
    <div className="page-wrap">
      ...
      <nav className="nav">
        ...
        <ul className="nav-links">
          ...
          <li><button className="nav-link nav-cta" onClick={() => setModal(true)}>Entrar / Cadastrar</button></li>
        </ul>
      </nav>
    ...
    <div>
          ...
          <li>
            <div className="nav-user-pill">
              <div className="nav-user-avatar">
                {avatarUrl ? <img src={avatarUrl} alt=""/> : profile.nome[0]}
              </div>
      ...
      <div className="dash">
        ...
        <main className="dash-main">
          {profile.tipo==="paciente"   && <DashPac  aba={aba} profile={profile} uid={user.id} onPU={onProfileUpdate} avatarUrl={avatarUrl}/>}
          {profile.tipo==="psicologo"  && <DashPsi  aba={aba} profile={profile} uid={user.id} onPU={onProfileUpdate} avatarUrl={avatarUrl}/>}
          {profile.tipo==="supervisor" && <DashSup  aba={aba} profile={profile} uid={user.id} onPU={onProfileUpdate} avatarUrl={avatarUrl}/>}
        </main>
      </div>
    </div>
  );
...
// ─── PACIENTE ────────────────────────────────────────────────────────────────
function DashPac({ aba, profile, uid, onPU, avatarUrl }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const {data:ps} = await supabase.from("profiles").select("id,nome,crp").eq("tipo","psicologo").or("status.eq.ativo,status.is.null");
    ...
    <div>
      ...
      <div className="prof-card">
        <div className="prof-av-wrap">
          <div className="prof-av">{avatarUrl ? <img src={avatarUrl} alt=""/> : profile.nome[0]}</div>
        </div>
  ...
  if (aba==="consultas") return (
    <div>
      ...
      {loading ? <div className="al al-info">Carregando...</div> :
       consultas.length===0 ? <div className="al al-info">Nenhuma consulta agendada ainda. Use o menu para agendar.</div> :
        <div className="panel">
          <div className="tw"><table>
            <thead><tr><th>Data</th><th>Horário</th><th>Psicólogo(a)</th><th>Sessão</th><th>Status</th></tr></thead>
            <tbody>{consultas.map(a=>(
              <tr key={a.id}><td>{fmt(a.data)}</td><td>{a.hora}</td><td>{a.psicologo_nome||"—"}</td>
    ...
    <div>
      ...
      {loading ? <div className="al al-info">Carregando...</div> : <>
        ...
        <div className="panel">
          ...
          {prox.length===0 ? <div className="al al-info">Nenhuma consulta agendada. Clique em <strong>Agendar Consulta</strong> no menu.</div> :
            <div className="tw"><table>
...
function TelaAgendar({ uid, profile, psicos, onAgendado }) {
  ...
  useEffect(() => {
    ...
    // Fetch disponibilidade se existir
    const diaSemana = new Date(ano,mes,dia).getDay();
    const dias = ["domingo","segunda","terca","quarta","quinta","sexta","sabado"];
    supabase.from("disponibilidade").select("horas").eq("psicologo_id",psico).eq("dia",dias[diaSemana]).single()
      .then(({data}) => { if(data?.horas) setDisp(data.horas); else setDisp(HORAS); });
  }, [dia, psico, mes, ano]);
  ...
  if (ok) return (
    <div>
      ...
      <div className="al al-ok" style={{fontSize:15,padding:"1.5rem",lineHeight:2}}>
        ...
        📅 Data: <strong>{fmt(ok.data)}</strong> · 🕐 <strong>{ok.hora}</strong><br/>
  ...
  return (
    <div>
      ...
      {psicos.length===0
        ? <div className="al al-warn">Nenhum psicólogo ativo no momento. Aguarde o supervisor ativar um profissional.</div>
        : <>
          <div className="panel">
...
// ─── PSICÓLOGO ───────────────────────────────────────────────────────────────
function DashPsi({ aba, profile, uid, onPU, avatarUrl }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const {data} = await supabase.from("disponibilidade").select("*").eq("psicologo_id",uid);
  ...
  async function salvarAgenda() {
    ...
    await supabase.from("disponibilidade").delete().eq("psicologo_id",uid);
    ...
    if (error) { toast("Erro ao salvar disponibilidade.", "err"); return; }
  ...
  if (aba==="agenda") return (
    <div>
      ...
      <div className="panel">
        ...
        <div className="al al-info" style={{marginBottom:"1rem"}}>Marque os horários em que você pode atender.</div>
        {DIAS.map(d=>(
          <div key={d} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid var(--border)",flexWrap:"wrap"}}>
            <span style={{width:70,fontWeight:600,fontSize:13.5,color:"var(--ink)"}}>{DIAS_LABEL[d]}</span>
            {HORAS.map(h=>(
              <label key={h} style={{display:"flex",alignItems:"center",gap:4,fontSize:13,cursor:"pointer",color:"var(--ink-2)"}}>
                <input type="checkbox" checked={(agenda[d]||[]).includes(h)} onChange={()=>toggleHora(d,h)}/> {h}
              </label>
            ))}
          </div>
        ))}
        <button className="btn btn-teal btn-sm" style={{marginTop:"1rem"}} onClick={salvarAgenda} disabled={savingAgenda}>
    ...
    <div>
      ...
      {loading ? <div className="al al-info">Carregando...</div> :
        <div className="panel">
          ...
          {atend.length===0 ? <div className="al al-info">Nenhum atendimento ainda.</div> :
            <div className="tw"><table>
              <thead><tr><th>Data</th><th>Horário</th><th>Paciente</th><th>Sessão</th><th>Status</th></tr></thead>
              <tbody>{atend.map(a=>(
                <tr key={a.id}><td>{fmt(a.data)}</td><td>{a.hora}</td><td>{a.paciente_nome||"—"}</td>
                  <td>#{a.sessao_numero}</td><td><span className={`st st-${a.status}`}>{a.status}</span></td></tr>
              ))}</tbody>
            </table></div>
          }
  ...
  if (aba==="feedbacks") return (
    <div>
      ...
      {loading ? <div className="al al-info">Carregando...</div> :
       fbs.length===0 ? <div className="al al-info">Nenhum feedback recebido ainda.</div> :
        fbs.map(fb=>(
      ...
      {loading ? <div className="al al-info">Carregando...</div> : <>
        ...
        <div className="panel">
          ...
          {atend.filter(a=>a.status!=="concluido"&&a.status!=="cancelado").length===0
            ? <div className="al al-info">Nenhum atendimento agendado.</div>
            : <div className="tw"><table>
...
// ─── SUPERVISOR ──────────────────────────────────────────────────────────────
function DashSup({ aba, profile, uid, onPU, avatarUrl }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const agUsers = ag||[];
      const psiNamesFromAg = [...new Set(agUsers.map(a=>({id:a.psicologo_id,nome:a.psicologo_nome})).filter(x=>x.id))];
      const pacNamesFromAg = [...new Set(agUsers.map(a=>({id:a.paciente_id,nome:a.paciente_nome})).filter(x=>x.id))];
      const usFromAg = [...psiNamesFromAg,...pacNamesFromAg].map(u=>({id:u.id,nome:u.nome,tipo:u===psiNamesFromAg.find(p=>p.id===u.id)?"psicologo":"paciente",status:"ativo"}));
      // Merge: usa us se tem dados, caso contrário usa os extraídos dos agendamentos
      const mergedUs = (us&&us.length>1) ? us : [...(us||[]), ...usFromAg.filter(u=>(us||[]).findIndex(x=>x.id===u.id)===-1)];
      const mergedPs = (ps&&ps.length>0) ? ps : psiNamesFromAg.map(p=>({id:p.id,nome:p.nome,status:"ativo"}));
      const {data:ms} = await supabase.from("contatos").select("*").order("created_at",{ascending:false});
      setTodos(ag||[]); setFbs(fb||[]); setPsicos(mergedPs); setUsuarios(mergedUs); setMensagens(ms||[]);
      setL(false);
    })();
  }, []);
  ...
  async function atualizar(id, status) {
    ...
    toast(`✅ Agendamento ${status === "confirmado" ? "confirmado" : status === "concluido" ? "concluído" : "cancelado"}!`);
  }
  ...
  async function enviarFb() {
    ...
    const {error} = await supabase.from("feedbacks").insert({ supervisor_id:uid, psicologo_id:fbPsico, supervisor_nome:profile.nome, psicologo_nome:pn, paciente_nome:fbPac||"—", nota:fbNota, texto:fbTxt });
    setFbB(false);
    if (error) { toast("Erro ao enviar feedback.", "err"); return; }
  ...
  async function setStatusUsuario(userId, novoStatus) {
    ...
    if (error && error.code !== "42703") { // 42703 = column does not exist
      toast("Erro ao atualizar usuário.", "err"); return;
    }
    setUsuarios(p => p.map(u => u.id === userId ? { ...u, status: novoStatus } : u));
    setPsicos(p => p.map(u => u.id === userId ? { ...u, status: novoStatus } : u));
    const acao = novoStatus === "ativo" ? "ativado" : novoStatus === "inativo" ? "desativado" : "aprovado";
    toast(`✅ Usuário ${acao} com sucesso!`);
  }

  async function marcarMsgLida(msg) {
    if (!msg.lida) {
      await supabase.from("contatos").update({lida:true}).eq("id",msg.id);
      setMensagens(p=>p.map(m=>m.id===msg.id?{...m,lida:true}:m));
    }
    setMsgSel(msg);
  }
    ...
    <div>
      ...
      {loading ? <div className="al al-info">Carregando...</div> : <>
        ...
        {usuarios.filter(u=>u.tipo==="supervisor"&&u.status==="pendente_aprovacao").length>0 && (
          <div className="panel">
            <div className="panel-title" style={{color:"var(--amber)"}}>⏳ Supervisores aguardando aprovação</div>
            {usuarios.filter(u=>u.tipo==="supervisor"&&u.status==="pendente_aprovacao").map(u=>(
              <div className="user-card" key={u.id}>
                <div className="user-card-left">
                  <div className="user-av-sm">{u.nome[0]}</div>
        ...
        <div className="panel">
          ...
          {usuarios.filter(u=>u.tipo==="psicologo").length===0
            ? <div className="al al-info">Nenhum psicólogo cadastrado ainda.</div>
            : usuarios.filter(u=>u.tipo==="psicologo").map(u=>(
              <div className="user-card" key={u.id}>
                <div className="user-card-left">
                  <div className="user-av-sm">{u.avatar_url ? <img src={u.avatar_url} alt=""/> : u.nome[0]}</div>
                  <div>
        ...
        <div className="panel">
          ...
          {usuarios.filter(u=>u.tipo==="paciente").length===0
            ? <div className="al al-info">Nenhum paciente cadastrado ainda.</div>
            : <div className="tw"><table>
    ...
    <div>
      ...
      {loading ? <div className="al al-info">Carregando...</div> : <>
        ...
        <div className="panel">
          ...
          {todos.length===0 ? <div className="al al-info">Nenhum agendamento.</div> :
            <div className="tw"><table>
      ...
      {loading ? <div className="al al-info">Carregando...</div> :
        <div className="panel">
          ...
          {todos.filter(a=>a.status!=="cancelado"&&a.status!=="concluido").length===0
            ? <div className="al al-ok">Nenhum agendamento pendente no momento.</div>
            : <div className="tw"><table>
                ...
                <tbody>{todos.filter(a=>a.status!=="cancelado"&&a.status!=="concluido").map(a=>(
                  <tr key={a.id}>
                    <td>{fmt(a.data)}</td><td>{a.hora}</td><td>{a.paciente_nome||"—"}</td><td>{a.psicologo_nome||"—"}</td>
      ...
      <div>
        ...
        {loading ? <div className="al al-info">Carregando...</div> : <>
          {msgSel ? (
            ...
            mensagens.length===0
              ? <div className="al al-info">Nenhuma mensagem recebida ainda. As mensagens enviadas pelo formulário "Fale Conosco" aparecerão aqui.</div>
              : mensagens.map(m=>(
          ...
          <div className="panel" style={{borderColor:"var(--amber)",borderWidth:2}}>
            ...
            {usuarios.filter(u=>u.tipo==="supervisor"&&u.status==="pendente_aprovacao").map(u=>(
              <div className="user-card" key={u.id}>
                <div className="user-card-left">
                  <div className="user-av-sm">{u.nome[0]}</div>
                  <div><div className="user-info-name">{u.nome}</div><div className="user-info-sub">{u.email}</div></div>
                </div>
                <div className="user-card-actions">
                  <button className="btn btn-sm btn-success" onClick={()=>setStatusUsuario(u.id,"ativo")}>✅ Aprovar</button>
                  <button className="btn btn-sm btn-danger"  onClick={()=>setStatusUsuario(u.id,"inativo")}>❌ Rejeitar</button>
